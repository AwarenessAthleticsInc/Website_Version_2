...
const UserList = (props) => {
    ...
    const users = props.data.filter((user) => {
        return user.roles === props.role;
    });
                        ...
                        .map((row, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TableCell key={`name-${row._id}`} align={props.align || 'center'}>{`${row.name.givenName} ${row.name.familyName}`}</TableCell>
                                    <TableCell key={`email-${row._id}`} align={props.align || 'center'}>{row.username}</TableCell>
                                    <TableCell key={`phone-${row._id}`} align={props.align || 'center'}>{`${row.phone}`}</TableCell>
                                    <TableCell key={`startDate-${row._id}`} align={props.align || 'center'}>{`${new Date(row.startDate).toLocaleDateString() || 'N/A'}`}</TableCell>
                                </TableRow>
                            );
                        })}
                    {emptyRows > 0 && (

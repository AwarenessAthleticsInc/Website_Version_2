                        ...
                        .map((row, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TableCell key={`${row._id}date`} align={props.align || 'center'}>{`${date === "Invalid Date" ? 'N/A' : date}`}</TableCell>
                                    <TableCell key={`${row._id}tournament`} align={props.align || 'center'}>{`${row.tournament.location.diamond}, ${row.tournament.location.city} (${row.tournament.dateTime ? row.tournament.dateTime.start.date : 'N/A'})`}</TableCell>
                                    <TableCell key={`${row._id}amount`} align={props.align || 'center'}>{row.team.team}</TableCell>
                                    <TableCell key={`${row._id}division`} align={props.align || 'center'}>{row.tournament.division || row.team.division || 'Co-ed'}</TableCell>
                                    <BalanceCell tournament={row} align={props.align} payments={payments} />
                                </TableRow>
                            );
                        })}
                    {emptyRows > 0 && (

...
const TournamentCard = (props) => {
    ...
    const [convenerId, setConvenerId] = useState(props.tournament && props.tournament.convener ? props.tournament.convener._id : '');
    const [tournament, setTournament] = useState({
        ...
        startDate: props.tournament ? new Date(props.tournament.dateTime.start.date).toDateString() : new Date().toDateString(),
        endTime: props.tournament ? props.tournament.dateTime.end.time : new Date().toTimeString(),
        endDate: props.tournament ? new Date(props.tournament.dateTime.end.date).toDateString() : new Date().toDateString(),
        minTeam: props.tournament ? props.tournament.teams.Min : '',
        maxTeam: props.tournament ? props.tournament.teams.Max : '',
        EntryDeadline: props.tournament ? new Date(props.tournament.dateTime.EntryDeadline) : new Date().toDateString(),
        ...
        convener: props.tournament ? props.tournament.convener : '',
        cancellationDate: props.tournament ? new Date(props.tournament.dateTime.cancellationDate) : new Date().toDateString()
    });
    const conveners = props.users.filter((user) => {
        return user.roles === 'admin' || user.roles === 'Convener'
    });
    ...
    const newTournamentHandler = (event) => {
        ...
        setTournament((prevs) => {
            return {
                ...
                convener: prevs.convener,
                cancellationDate: prevs.cancellationDate
            }
        });
    }
    ...
    const tournamentConvenerHandler = (event) => {
        const id = event.target.value;
        const convener = conveners.filter((user) => {
            return user._id === id;
        })
        setTournament((prevs) => {
            return {
    ...
    const setStartDate = (date) => {
        setTournament((prevs) => {
            return {
                ...
                cost: prevs.cost,
                type: prevs.type,
                startTime: new Date(date).toTimeString(),
    ...
    const setEndDate = (date) => {
        setTournament((prevs) => {
            return {
                ...
                startTime: prevs.startTime,
                startDate: prevs.startDate,
                endTime: new Date(date).toTimeString(),
    ...
    const setCancellationDate = (date) => {
        const newDate = new Date(date).toDateString();
        setTournament((prevs) => {
            return {
                images: prevs.images,
    ...
    const checkCompletion = () => {
        return new Promise((resolve, reject) => {
            ...
            if (tournament.city === '' || tournament.diamond === '' || tournament.province === '' || tournament.FullAddress === '' || tournament.maxTeam === '' || tournament.minTeam === '') {
    ...
    const save = () => {
        checkCompletion().then(() => {
            const startDay = new Date(tournament.startDate).getDate() + 1; //starts at zero so plus one for proper date
            const startMonth = Number(new Date(tournament.startDate).getMonth() + 1).toString(); //starts at zero so plus one for proper date
    ...
    return <Backdrop sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1 }} open={true}>
        <Paper sx={{ m: { xs: '1%', md: '10%' }, width: { xs: '98%', md: '80%' }, p: '1rem', overflowY: 'scroll', maxHeight: '95%' }}>
            ...
            <Box sx={{ p: '1rem' }}>
                ...
                <hr />
                <Typography variant="h6" component="h6" gutterBottom>Convener in charge</Typography>
                <FormControl sx={{ width: '100%', m: '1% 0' }} variant="standard">
                    <InputLabel id="convener">Select Convener</InputLabel>
                    <Select
                        id="convener"
                        value={convenerId}
                        onChange={tournamentConvenerHandler}
                        label="Convener"
                    >
                        {conveners.map((user, index) => {
                            return <MenuItem key={index} value={user._id}>{`${user.name.givenName} ${user.name.familyName}`}</MenuItem>
                        })}
                    </Select>
                </FormControl>
                <hr />
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} md={6}>
                            <DesktopDatePicker
                                ...
                                inputFormat="MM/dd/yyyy"
                                value={tournament.cancellationDate}

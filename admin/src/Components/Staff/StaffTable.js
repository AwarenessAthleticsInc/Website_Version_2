                ...
                <TableBody>
                    {stableSort(props.data, getComparator(order, orderBy))
                        ...
                        .map((row, index) => {
                            ...
                            const array = row.assets.profileImage.split('/');
                            const name = array[1];
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TableCell key={`title-${row._id}`} align={props.align || 'center'}>
                                        <Avatar alt={`${row.details.name.givenName} ${row.details.name.familyName}`} src={`/${location}/${name}/200/${name}.webp`} />
                                    </TableCell>
                                    <TableCell key={`location-${row._id}`} align={props.align || 'center'}>{`${row.details.name.givenName} ${row.details.name.familyName}`}</TableCell>
                                    <TableCell key={`registered-${row._id}`} align={props.align || 'center'}>{`${row.work.title}`}</TableCell>
                                    <TableCell key={`left-${row._id}`} align={props.align || 'center'}>{`${row.details.username}`}</TableCell>
                                </TableRow>
                            );
                        })}
                    {emptyRows > 0 && (

...
const RegisterButton = (props) => {
    ...
    var CancellationDate = new Date(props.tournament.dateTime.EntryDeadline);
    var today = new Date();
    useEffect(() => {
                ...
                {division.map((div) => {
                    ...
                    if (check) {
                        ...
                        if (today.toLocaleDateString('us-en') !== CancellationDate.toLocaleDateString('us-en') && CancellationDate < today) {
                            return <button disabled className={`rounded-pill btn btn-primary m-1 w-100 register regButton-${props.tournament._id}`} value={props.tournament._id}><AccessTimeIcon /> Deadline Passed</button>
                        }
                        return <button onClick={() => {props.onClick(div)}} id={div} className={`rounded-pill btn btn-outline-primary w-100 m-1 register regButton-${props.tournament._id}`} value={props.tournament._id}>{div}</button>
                    }
                })}
            </Box>
        }

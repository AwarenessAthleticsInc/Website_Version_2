...
const TeamGrowthChart = (props) => {
    ...
    useEffect(() => {
        $.ajax({
            type: 'get',
            url: '/dashboard/metrics/teamGrowth',
        }).then((responseYears) => {
    ...
    return <Paper sx={{ p: '0.8rem', display: 'flex', flexDirection: 'column', height: 'auto', aspectRatio: '8/5.15' }}>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Typography variant="h6" gutterBottom>{`New Teams for ${currentYear}`}</Typography>
        ...
        {currentYear === 2022 && <Typography variant="p" gutterBottom>{`${years[0][12].length} teams aren't tracked on the chart above`}</Typography>}
    </Paper>
}
